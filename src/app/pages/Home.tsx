...
// Reports tab — the reports directory (grouped by category) plus the selected
// report opened inline. A report = the module's list pre-filtered for that
// report (Marek call #21). Reports live here as a Home tab, not a separate
// section.
function ReportsTab() {
  const [selectedId, setSelectedId] = useState<string | null>(null);
  return selectedId ? (
    <ReportView reportId={selectedId} embedded onBack={() => setSelectedId(null)} />
  ) : (
    <ReportsLanding onOpen={setSelectedId} />
  );
}
      ...
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[13px] text-[#6B7280] mt-0.5">
            {safeTab === "Reports" ? "Pre-filtered reports — generate, print, share or schedule any report." : "Overview of your business performance"}
          </p>
        </div>
