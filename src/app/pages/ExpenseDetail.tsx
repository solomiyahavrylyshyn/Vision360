...
export function ExpenseDetail() {
  ...
  // Documents — the SAME shared gallery (search · filters · sort · upload) used on
  // estimates, jobs and clients. Seeded per expense id; uploads append via onChange.
  const [documents, setDocuments] = useState<PreviewableDoc[]>(() =>
    ((id && expenseDocsById[id]) || []).map((d, i) => ({
      id: `seed-${id}-${i}`,
      name: `${d.label}.jpg`,
      size: "—",
      date: expense?.date ?? "",
    ...
    <div className="min-h-screen bg-[#F5F7FA]">
      ...
      <section className="mx-6 mb-6 rounded-xl border border-[#E5E7EB] bg-white p-4">
        ...
        <ResizablePanelGroup direction="horizontal" className="mt-4 min-h-[360px] items-stretch">
          ...
          <div className="h-full rounded-xl border border-[#E5E7EB] bg-white p-4">
          ...
          {/* Col 2 — Documents: the shared gallery (search · filters · sort · upload),
              identical to estimates / jobs / clients. */}
          <ResizablePanel defaultSize={50} minSize={30} className="min-w-0">
          <div className="h-full rounded-xl border border-[#E5E7EB] bg-white p-4">
            <DocumentsGallery documents={documents} onChange={setDocuments} />
          </div>
          </ResizablePanel>
